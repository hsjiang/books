...
class DividerItemDecoration(val context: Context, val orientation: Int) : RecyclerView.ItemDecoration() {
    companion object {
    ...
    fun getItemDecoration(): Decoration {
        val width = (context as? Activity)?.windowManager?.defaultDisplay?.width

        return Decoration(width ?: 1024, 1)
    }

    class Decoration(val width: Int, val height: Int, @ColorInt val color: Int = Color.LTGRAY) {
